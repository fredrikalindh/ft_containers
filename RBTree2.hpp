...
//######################### CONSTRUCTORS #######################################
	...
	value_type& operator[] (const value_type& k){ // find node or parent to where it should be?
		return add(k)->value_;
	}
//########################### MODIFIERS #######################################
	void					swap (RB_Tree& x) {
...
// private:
	...
	void deleteKey(value_type& value){
		// if (!(count(toDelete)))
		if (!isRed(root_->left_) && !isRed(root_->right_))
			root_->color_ = RED;
	...
	Node	*deleteKey(Node *x, value_type& value){
		if (comp(value, x->value_)){
			...
			if (isRed(x->left_))
				x = rotateRight(x);
			if (x->value_ == value && !x->right_){ //delete here
				--size_;
				delete x;
				return 0;
			}
...
class rb_tree_iterator
{
	...
	Tree&	tree_;
	Node*	node_;
	rb_tree_iterator(Tree &tree, Node *node = 0):tree_(tree), node_(node){}
