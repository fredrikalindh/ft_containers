...
class map
{
	struct Node
	{
		...
		ft::pair<Key, T>	pair_;
		Node*				parent_ ,leftChild_, rightChild_;
		bool				isRed, isLeft;
...
// ############################## ITERATOR #####################################
	...
	public:
		...
		bidirectional_iterator& operator++(void){
			...
			}else if (node_->isLeft && !node_->rightChild_){
				node_ = node_->parent_;
		...
		bidirectional_iterator& operator--(void){
			...
			else if (!node_->isLeft && !node_->leftChild_)
				node_ = node_->parent_;
...
//######################### CONSTRUCTORS #######################################
	...
	mapped_type& operator[] (const key_type& k){ // find node or parent to where it should be?
		...
		if (!(node = find_node(root_, k)))
			...
			std::cout << "found " << k << node->pair_.first << '\n';
...
private:
	Node 	*add(ft::pair<Key, T> toAdd){
		Node *added;
		if (root_ == 0){
			root_ = new Node(toAdd, 0, true, false);
			added = root_;
			++size_;
		}else {
	...
	Node	*add(Node *node, ft::pair<Key, T> toAdd){
		if (!comp(node->pair_.first, toAdd.first)){
			if (node->leftChild_)
	...
	bool AuntIsRed(Node *node){
		if (node->parent_->isLeft){
			if (node->parent_->parent_->rightChild_ == 0)
				return false;
			return node->parent_->parent_->rightChild_->isRed;
		}
		if (node->parent_->parent_->leftChild_ == 0)
			return false;
	...
	// bool 	fixUnevenBlack(Node *problem) {
	// 	if (problem->isRed) {
	// 		problem->isRed = false;
	// 		return true;
	// 	}
	// 	if (problem->leftChild_){
	// 		if (fixUnevenBlack(problem->leftChild_))
	// 			return true;
	// 	}
	// 	else if (problem->rightChild_){
	// 		if (fixUnevenBlack(problem->leftChild_)){
	// 			return true;
	// 		}
	// 	}
